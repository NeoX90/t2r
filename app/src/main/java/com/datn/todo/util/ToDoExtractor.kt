...
object ToDoExtractor {
    ...
    fun extractVietnameseTimeAndDateSecondFilter(string: String): String? {
        if (string.contains("giờ") && string.contains("phút")) {
            ...
            val minute =
                string.substringAfterLast("phút").substringBefore("ngày").trim().toIntOrNull()
                    ?: string.substringAfterLast("phút").substringBefore("hôm").trim().toIntOrNull() ?: return null
            ...
            } else if (string.lowercase().contains("hôm nay")) {
                val today = LocalDate.now()
                return if (minute < 10) {
                    "$hour:0${minute}/${today.format(DateTimeFormatter.ofPattern(DATE_PATTERN))}"
                } else {
                    "$hour:${minute}/${today.format(DateTimeFormatter.ofPattern(DATE_PATTERN))}"
                }
